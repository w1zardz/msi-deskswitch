...
from install import install, merge


def matches(rule, key, modifiers, device=(6645, 4141), app="com.apple.Safari", source=SOURCES[0]):
...
class ProfileTests(unittest.TestCase):
    ...
    def test_page_down_fires_only_after_release(self):
        rule = self.lookup("page_down")
        self.assertEqual(rule["to"], [])
        self.assertEqual(rule["to_after_key_up"], [{"key_code": "f11", "modifiers": ["control", "shift"]}])
        self.assertIsNone(self.lookup("page_down", ["control"]))


class InstallerTests(unittest.TestCase):
    def test_new_profile(self):
        result, name = merge({}, None)
